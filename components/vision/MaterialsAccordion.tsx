  ...
  return (
    <div className="space-y-4">
      {Object.entries(grouped).map(([category, items]) => {
        ...
        return (
          <div key={category} className="overflow-hidden rounded-[1.5rem] border border-hairline bg-white shadow-[0_12px_32px_rgba(15,23,42,0.06)]">
            <button
            ...
            >
              <div>
                ...
                <div className="mt-1 text-sm text-ink-500">{items.length} item{items.length !== 1 ? 's' : ''} • {formatCurrencyRange(groupLow, groupHigh)}</div>
              </div>
              {openSections[category] ? <ChevronUp className="h-5 w-5 text-ink-400" /> : <ChevronDown className="h-5 w-5 text-ink-400" />}
            </button>
                ...
                {items.map((item, index) => (
                  <div key={`${item.item}-${index}`} className="rounded-[1.25rem] border border-hairline bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-base font-semibold text-ink">{item.item}</div>
                        <div className="mt-1 text-sm text-ink-500">{item.quantity} {item.unit} • {item.finish_tier} tier</div>
                      </div>
                    ...
                    {item.sourcing_notes && (
                      <p className="mt-3 text-sm leading-relaxed text-ink-600">{item.sourcing_notes}</p>
                    )}

                    <div className="mt-4 flex flex-wrap gap-2">
                      <span className="inline-flex items-center gap-1 rounded-full bg-canvas-100 px-3 py-1 text-xs font-medium text-ink-600">
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-1 rounded-full border border-hairline px-3 py-1 text-xs font-medium text-ink-600 transition-colors hover:bg-canvas-50"
                      >
      ...
      {materials.sourcing_notes && (
        <div className="rounded-[1.5rem] border border-hairline bg-white p-5 text-sm text-ink-600 shadow-[0_12px_32px_rgba(15,23,42,0.06)]">
          <span className="font-semibold text-ink">Sourcing note:</span> {materials.sourcing_notes}
        </div>
      )}
    </div>
  );
